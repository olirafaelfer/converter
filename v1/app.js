    ...
    bodyHtml: `
      <div class="space-y-3">
        <div class="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
          ...
          <div class="mt-1 text-sm font-semibold text-slate-100">${safeText(item.title)}</div>
          <div class="mt-3 grid grid-cols-2 gap-3">
            <div>
              <div class="text-xs text-slate-400">Fonte</div>
              <div class="mt-1 text-sm text-slate-200">${safeText(item.source)}</div>
            </div>
            <div>
              <div class="text-xs text-slate-400">Duração</div>
              <div class="mt-1 text-sm text-slate-200">${safeText(item.duration)}</div>
            </div>
          </div>
        </div>
        <p class="text-xs text-slate-400">
          Dica: depois a gente liga isso ao backend (busca real + parse + formatos).
        </p>
      </div>
    `,
    footerButtons: [
...
function openAdblockModal() {
  setModal({
    ...
    subtitle: "Para manter o site gratuito, precisamos de anúncios ativos",
    icon: "shield",
    bodyHtml: `
      <div class="space-y-3">
        <p>Desative o AdBlock (ou coloque este site na whitelist) para liberar downloads.</p>
        <div class="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 text-sm">
          <div class="font-semibold text-slate-100">Sugestões rápidas</div>
          <ul class="mt-2 list-disc pl-5 text-slate-200/90 space-y-1">
            <li>Recarregue a página depois de desativar.</li>
            <li>Se estiver testando, use “Alternar AdBlock” para simular o comportamento.</li>
          </ul>
        </div>
      </div>
    `,
...
function openPremiumModal() {
  setModal({
    title: "Premium (mock)",
    subtitle: "Fluxo visual — pagamento entra depois no backend",
    icon: "star",
    bodyHtml: `
      <div class="space-y-4">
        <div class="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
          <div class="flex items-center justify-between">
            <div>
              <div class="text-sm font-semibold text-slate-100">Mensal</div>
              <div class="text-xs text-slate-400">Sem anúncios + downloads diretos</div>
            </div>
            <div class="text-sm font-semibold text-indigo-100">R$ 14,90</div>
          </div>
        </div>
        <div class="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
          <div class="flex items-center justify-between">
            <div>
              <div class="text-sm font-semibold text-slate-100">Anual</div>
              <div class="text-xs text-slate-400">Melhor custo-benefício</div>
            </div>
            <div class="text-sm font-semibold text-indigo-100">R$ 149,00</div>
          </div>
        </div>
...
function openRewardedAdModal(item, fmt) {
    ...
    <div class="space-y-4">
      ...
      <div class="rounded-2xl bg-slate-900/40 p-4 ring-1 ring-white/10">
        ...
        <div class="mt-4 h-2 w-full overflow-hidden rounded-full bg-white/5 ring-1 ring-white/10">
          <div id="adBar" class="h-full rounded-full bg-indigo-400/60" style="width:${Math.round((1 - (remaining/seconds)) * 100)}%"></div>
        </div>
      ...
      <div class="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
        <div class="text-xs text-slate-400">Por que isso?</div>
        <div class="mt-1 text-sm text-slate-200">Assim os anúncios ficam menos invasivos e você só vê quando quer baixar.</div>
      </div>
    </div>
  `;
  ...
  setModal({
    ...
    subtitle: "Assista para desbloquear",
    icon: "play",
    bodyHtml: body(),
    footerButtons: [btnCancel, btnUnlock]
  });

  const footer = $("#modalFooter");
  const unlockBtnEl = footer.querySelectorAll("button")[1];
  ...
  function tick() {
    ...
    if (bar) bar.style.width = `${Math.round((1 - (remaining/seconds)) * 100)}%`;

    if (remaining <= 0) {
      clearInterval(timer);
      unlockBtnEl.textContent = "Liberar agora";
      unlockBtnEl.className = "rounded-2xl bg-indigo-500/25 px-5 py-2.5 text-sm font-semibold text-indigo-100 ring-1 ring-indigo-400/30 hover:bg-indigo-500/35";
      unlockBtnEl.onclick = () => {
        closeModal();
